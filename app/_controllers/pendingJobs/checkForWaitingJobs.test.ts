...
describe('checkForWaitingJobs', () => {
  ...
  it('should return early if max concurrent jobs reached', async () => {
    ...
    const mockPendingJobs = Array(AppConstants.MAX_CONCURRENT_JOBS).fill({
      ...
      ; (getPendingImagesByStatusFromAppState as jest.Mock).mockImplementation((statuses) => {
  ...
  it('should handle missing image request', async () => {
    ...
    const mockWaitingJob = { artbot_id: 'test-id', status: JobStatus.Waiting }
      ...
      ; (transitionJobFromWaitingToRequested as jest.Mock).mockResolvedValue(mockWaitingJob)
  ...
  it('should handle successful API response', async () => {
    ...
    const mockCheckResponse = {
      ...
      ; (transitionJobFromWaitingToRequested as jest.Mock).mockResolvedValue(mockWaitingJob)
      ; (getImageRequestsFromDexieById as jest.Mock).mockResolvedValue([mockImageRequest])
      ; (ImageParamsForHordeApi.build as jest.Mock).mockResolvedValue({ apiParams: mockApiParams })
    ...
    expect(updatePendingImage).toHaveBeenCalledWith('test-id', expect.objectContaining({
  ...
  it('should handle unexpected errors', async () => {
    ...
    const mockImageRequest = { id: 'request-id' }
      ...
      ; (transitionJobFromWaitingToRequested as jest.Mock).mockResolvedValue(mockWaitingJob)
      ; (getImageRequestsFromDexieById as jest.Mock).mockResolvedValue([mockImageRequest])
      ; (ImageParamsForHordeApi.build as jest.Mock).mockRejectedValue(new Error('Unexpected error'))
