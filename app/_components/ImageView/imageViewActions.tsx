...
function ImageViewActions({
  ...
  const handleDelete = useCallback(
    async ({ deleteAll = false } = {} as { deleteAll?: boolean }) => {
      NiceModal.show('delete', {
        children: (
          <DeleteConfirmation
            deleteButtonTitle={
              deleteAll
                ? `Delete ${imageFiles.length} images`
                : 'Delete this image'
            }
            ...
            message={
              <>
                <p>
                  ...
                  {deleteAll
                    ? `*ALL* images in this batch? (${imageFiles.length} images)`
                    : 'this image?'}
                </p>
            ...
            onDelete={async () => {
              if (!deleteAll) {
                await deleteImageFromDexie(
                  (currentImageId as string) || (imageId as string)
                );
                await onDelete();
              } else {
